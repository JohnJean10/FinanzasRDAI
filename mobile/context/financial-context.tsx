...
import { AppData, Transaction, Goal, Debt, BudgetConfig } from '../lib/types';
...
export function FinancialProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const stored = await AsyncStorage.getItem('finanzasrd_data_v2');
                if (stored) {
    ...
    useEffect(() => {
        const saveData = async () => {
            if (!isLoading) {
                try {
                    await AsyncStorage.setItem('finanzasrd_data_v2', JSON.stringify(data));
                } catch (e) {
    ...
    }, [data.budgetConfigs, data.user.monthsOfPeace, isLoading]);
