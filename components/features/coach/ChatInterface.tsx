    ...
    const handleExecuteAction = (msgId: string, action: ActionCommand) => {
        try {
            if (action.type === 'ADD_TRANSACTION') {
                ...
                const targetName = action.payload.goalName || action.payload.name || '';
                const targetGoal = goals.find(g => g.name.toLowerCase().includes(targetName.toLowerCase()));
                ...
                if (targetGoal) {
                    ...
                    // Fallback if AI hallucinates a non-existent goal name
                    addTransaction({
                        amount: action.payload.amount,
                        category: 'ahorro',
                        type: 'expense',
