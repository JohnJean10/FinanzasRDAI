...
import { X, Save, Calendar, Repeat, CreditCard } from "lucide-react";
import { useFinancial } from "@/lib/context/financial-context";
import { Transaction } from "@/lib/types";

interface AddTransactionModalProps {
    // Props are now optional as visibility is controlled by context
    isOpen?: boolean;
    onClose?: () => void;
...
export function AddTransactionModal({ isOpen: propIsOpen, onClose: propOnClose, editingTransaction }: AddTransactionModalProps) {
    ...
    // Determine visibility based on props (legacy/direct usage) or context (global usage)
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className={`w-full text-3xl font-bold p-2 bg-transparent border-b-2 outline-none transition-colors placeholder:text-slate-300 dark:placeholder:text-slate-700 text-slate-900 dark:text-white ${formData.type === 'income' ? 'border-emerald-500/50 focus:border-emerald-500' : 'border-red-500/50 focus:border-red-500'}`}
                            placeholder="0.00"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder={formData.type === 'income' ? "Ej. Nómina Enero" : "Ej. Supermercado"}
                        />
                    </div>
